...
class Controller:
    ...
    def one_cycle(self, query: str):
        def run():
            ...
            music_list_infos, artist, artist_img_url, artist_url = self.query(
                query=query)
    ...
    def download_songs(self, music_list_infos: list, artist: str, artist_img_url: str, artist_url: str):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_thread) as executor:
            ...
            # summary
            self.mysql.save_summary(artist=artist, summary=executor.submit(
                query_summary, artist).result())
        ...
        for song in music_list_infos:
            ...
            download_song_infos.append({
                "artist": artist,
                'title': clear_str(title=song['title'], artist=artist),
        ...
        # 分割
        music_ID_list_chunks = [download_song_infos[x:x+self.max_dow_thread*2]
                                for x in range(0, len(download_song_infos), 10)]
