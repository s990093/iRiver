...
def get_music_list(request):
    ...
    for row in r:
        result_dict = {'artist': row[1], 'title': row[2], 'music_ID': row[3],
                       'artist_url': row[4], 'keywords': row[5], 'views': row[6], 'publish_time': row[7]}
...
def download_song(request):
    ...
    path = os.path.join(
        "media", song_info['artist'], "songs", f"{song_info['music_ID']}.mp3")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        ...
        music_ID_info = executor.submit(
            get_music_ID_info, song_info['music_ID']).result()

        # dow summary
        summary = executor.submit(
            query_summary, song_info['artist'])  # 取得歌手維基百科摘要
...
def download_songs(request):
    ...
    music_ID_list_chunks = [music_list[x:x+8]
                            for x in range(0, len(music_list), 10)]
